...
android {
    ...
    defaultConfig {
        ...
        resourceConfigurations.addAll(supportedLocales.split(","))
    ...
    flavorDimensions.addAll(listOf("dependencies", "automotive"))
    productFlavors {
        create("foss") {
        ...
        create("automotive") {
            ...
            versionCode = (defaultConfig.versionCode ?: 0) + 1
            minSdk = 29
        }
    }
    ...
    lint {
        ...
        warning.addAll(listOf("MissingTranslation"))
    }
    ...
    applicationVariants.all {
        ...
        val goingelectricKey = env["GOINGELECTRIC_API_KEY"] ?: project.findProperty("GOINGELECTRIC_API_KEY") as String?
        if (goingelectricKey != null) {
            resValue("string", "goingelectric_key", goingelectricKey)
        }
        val openchargemapKey = env["OPENCHARGEMAP_API_KEY"] ?: project.findProperty("OPENCHARGEMAP_API_KEY") as String? ?: run {
            if (project.hasProperty("OPENCHARGEMAP_API_KEY_ENCRYPTED")) {
                decode(project.findProperty("OPENCHARGEMAP_API_KEY_ENCRYPTED") as String, "FmK.d,-f*p+rD+WK!eds")
            } else null
        }
        if (openchargemapKey != null) {
            resValue("string", "openchargemap_key", openchargemapKey)
        }
        val googleMapsKey = env["GOOGLE_MAPS_API_KEY"] ?: project.findProperty("GOOGLE_MAPS_API_KEY") as String?
        if (googleMapsKey != null && flavorName.startsWith("google")) {
            resValue("string", "google_maps_key", googleMapsKey)
        }
        val mapboxKey = env["MAPBOX_API_KEY"] ?: project.findProperty("MAPBOX_API_KEY") as String? ?: run {
            if (project.hasProperty("MAPBOX_API_KEY_ENCRYPTED")) {
                decode(project.findProperty("MAPBOX_API_KEY_ENCRYPTED") as String, "FmK.d,-f*p+rD+WK!eds")
            } else null
        }
        if (mapboxKey != null) {
            resValue("string", "mapbox_key", mapboxKey)
        }
        val chargepriceKey = env["CHARGEPRICE_API_KEY"] ?: project.findProperty("CHARGEPRICE_API_KEY") as String? ?: run {
            if (project.hasProperty("CHARGEPRICE_API_KEY_ENCRYPTED")) {
                decode(project.findProperty("CHARGEPRICE_API_KEY_ENCRYPTED") as String, "FmK.d,-f*p+rD+WK!eds")
            } else null
        }
        if (chargepriceKey != null) {
            resValue("string", "chargeprice_key", chargepriceKey)
        }
        val fronyxKey = env["FRONYX_API_KEY"] ?: project.findProperty("FRONYX_API_KEY") as String? ?: run {
            if (project.hasProperty("FRONYX_API_KEY_ENCRYPTED")) {
                decode(project.findProperty("FRONYX_API_KEY_ENCRYPTED") as String, "FmK.d,-f*p+rD+WK!eds")
            } else null
        }
        if (fronyxKey != null) {
            resValue("string", "fronyx_key", fronyxKey)
        }
        val acraKey = env["ACRA_CRASHREPORT_CREDENTIALS"] ?: project.findProperty("ACRA_CRASHREPORT_CREDENTIALS") as String? ?: run {
            if (project.hasProperty("ACRA_CRASHREPORT_CREDENTIALS_ENCRYPTED")) {
                decode(project.findProperty("ACRA_CRASHREPORT_CREDENTIALS_ENCRYPTED") as String, "FmK.d,-f*p+rD+WK!eds")
            } else null
        }
        if (acraKey != null) {
