...
android {
    ...
    applicationVariants.all {
        ...
        var goingelectricKey = env["GOINGELECTRIC_API_KEY"] ?: project.findProperty("GOINGELECTRIC_API_KEY") as String?
        if (goingelectricKey != null) {
            resValue("string", "goingelectric_key", goingelectricKey)
        }
        
        var openchargemapKey = env["OPENCHARGEMAP_API_KEY"] ?: project.findProperty("OPENCHARGEMAP_API_KEY") as String?
        if (openchargemapKey == null && project.hasProperty("OPENCHARGEMAP_API_KEY_ENCRYPTED")) {
            openchargemapKey = decode(project.findProperty("OPENCHARGEMAP_API_KEY_ENCRYPTED") as String, "FmK.d,-f*p+rD+WK!eds")
        }
        if (openchargemapKey != null) {
            resValue("string", "openchargemap_key", openchargemapKey)
        }
        
        var googleMapsKey = env["GOOGLE_MAPS_API_KEY"] ?: project.findProperty("GOOGLE_MAPS_API_KEY") as String?
        if (googleMapsKey != null && flavorName.startsWith("google")) {
            resValue("string", "google_maps_key", googleMapsKey)
        }
        
        var mapboxKey = env["MAPBOX_API_KEY"] ?: project.findProperty("MAPBOX_API_KEY") as String?
        if (mapboxKey == null && project.hasProperty("MAPBOX_API_KEY_ENCRYPTED")) {
            mapboxKey = decode(project.findProperty("MAPBOX_API_KEY_ENCRYPTED") as String, "FmK.d,-f*p+rD+WK!eds")
        }
        if (mapboxKey != null) {
            resValue("string", "mapbox_key", mapboxKey)
        }
        
        var chargepriceKey = env["CHARGEPRICE_API_KEY"] ?: project.findProperty("CHARGEPRICE_API_KEY") as String?
        if (chargepriceKey == null && project.hasProperty("CHARGEPRICE_API_KEY_ENCRYPTED")) {
            chargepriceKey = decode(project.findProperty("CHARGEPRICE_API_KEY_ENCRYPTED") as String, "FmK.d,-f*p+rD+WK!eds")
        }
        if (chargepriceKey != null) {
            resValue("string", "chargeprice_key", chargepriceKey)
        }
        
        var fronyxKey = env["FRONYX_API_KEY"] ?: project.findProperty("FRONYX_API_KEY") as String?
        if (fronyxKey == null && project.hasProperty("FRONYX_API_KEY_ENCRYPTED")) {
            fronyxKey = decode(project.findProperty("FRONYX_API_KEY_ENCRYPTED") as String, "FmK.d,-f*p+rD+WK!eds")
        }
        if (fronyxKey != null) {
            resValue("string", "fronyx_key", fronyxKey)
        }
        
        var acraKey = env["ACRA_CRASHREPORT_CREDENTIALS"] ?: project.findProperty("ACRA_CRASHREPORT_CREDENTIALS") as String?
        if (acraKey == null && project.hasProperty("ACRA_CRASHREPORT_CREDENTIALS_ENCRYPTED")) {
            acraKey = decode(project.findProperty("ACRA_CRASHREPORT_CREDENTIALS_ENCRYPTED") as String, "FmK.d,-f*p+rD+WK!eds")
        }
        if (acraKey != null) {
...
fun decode(s: String, key: String): String {
    return String(xorWithKey(java.util.Base64.getDecoder().decode(s), key.toByteArray()), Charsets.UTF_8)
}
