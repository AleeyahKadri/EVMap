...
android {
    defaultConfig {
        ...
        resConfigs(*supportedLocales.split(",").toTypedArray())
    ...
    extensions.configure<pt.jcosta.resourceplaceholders.plugin.ResourcePlaceholdersPluginExtension>("resourcePlaceholders") {
    ...
    applicationVariants.all {
        ...
        val goingelectricKey = env["GOINGELECTRIC_API_KEY"] ?: project.findProperty("GOINGELECTRIC_API_KEY")?.toString()
        if (goingelectricKey != null) {
            resValue("string", "goingelectric_key", goingelectricKey)
        }

        var openchargemapKey = env["OPENCHARGEMAP_API_KEY"] ?: project.findProperty("OPENCHARGEMAP_API_KEY")?.toString()
        ...
        val googleMapsKey = env["GOOGLE_MAPS_API_KEY"] ?: project.findProperty("GOOGLE_MAPS_API_KEY")?.toString()
        if (googleMapsKey != null && flavorName.startsWith("google")) {
            resValue("string", "google_maps_key", googleMapsKey)
        }

        var mapboxKey = env["MAPBOX_API_KEY"] ?: project.findProperty("MAPBOX_API_KEY")?.toString()
        ...
        var chargepriceKey = env["CHARGEPRICE_API_KEY"] ?: project.findProperty("CHARGEPRICE_API_KEY")?.toString()
        ...
        var fronyxKey = env["FRONYX_API_KEY"] ?: project.findProperty("FRONYX_API_KEY")?.toString()
        ...
        var acraKey = env["ACRA_CRASHREPORT_CREDENTIALS"] ?: project.findProperty("ACRA_CRASHREPORT_CREDENTIALS")?.toString()
...
dependencies {
    ...
    add("normalImplementation", "androidx.car.app:app-projected:$carAppVersion")
    add("automotiveImplementation", "androidx.car.app:app-automotive:$carAppVersion")

    // AnyMaps
    val anyMapsVersion = "8f1226e1c5"
    implementation("com.github.ev-map.AnyMaps:anymaps-base:$anyMapsVersion")
    add("googleImplementation", "com.github.ev-map.AnyMaps:anymaps-google:$anyMapsVersion")
    add("googleImplementation", "com.google.android.gms:play-services-maps:18.1.0")
    ...
    add("googleImplementation", "com.mapbox.mapboxsdk:mapbox-android-core:2.0.1")
    // patched version that removes build-time dependency on GMS (-> no Google location services)
    add("fossImplementation", "com.github.ev-map:mapbox-events-android:a21c324501")

    // Google Places
    add("googleImplementation", "com.google.android.libraries.places:places:3.2.0")
    add("googleImplementation", "org.jetbrains.kotlinx:kotlinx-coroutines-play-services:1.7.1")
    ...
    add("googleImplementation", "com.android.billingclient:billing:$billingVersion")
    add("googleImplementation", "com.android.billingclient:billing-ktx:$billingVersion")
    ...
    // testing for car app
    add("testGoogleImplementation", "androidx.car.app:app-testing:$carAppVersion")
    add("testGoogleImplementation", "androidx.test:core:1.5.0")
